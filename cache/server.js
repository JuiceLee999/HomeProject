...
function mapBarcodeCategory(cat) {
  const c = (cat || '').toLowerCase();
  if (/electronic|computer|phone|camera|audio|video/.test(c)) return 'Electronics';
  if (/tool|hardware|fastener|power tool/.test(c)) return 'Tools';
  if (/food|beverage|drink|grocery|snack|kitchen/.test(c)) return 'Kitchen';
  if (/clothing|apparel|shoe|wear|fashion/.test(c)) return 'Clothing';
  if (/sport|outdoor|fitness|camping/.test(c)) return 'Sports';
  if (/book|magazine|media/.test(c)) return 'Books';
  if (/toy|game|puzzle/.test(c)) return 'Toys';
  if (/office|school|stationery/.test(c)) return 'Office';
  if (/auto|car|vehicle|truck/.test(c)) return 'Automotive';
  if (/furniture|decor|home|bedding/.test(c)) return 'Furniture';
  return 'Other';
}

app.get('/api/barcode/:code', verifyToken, async (req, res) => {
  const code = req.params.code.replace(/[^0-9A-Za-z]/g, '');
  if (!code) return res.status(400).json({ error: 'Invalid barcode' });

  const withTimeout = (p, ms) => Promise.race([p, new Promise((_, r) => setTimeout(() => r(new Error('timeout')), ms))]);

  // Try UPC Item DB (free tier — no key needed, 100 req/day)
  try {
    const r = await withTimeout(fetch(`https://api.upcitemdb.com/prod/trial/lookup?upc=${code}`), 5000);
    if (r.ok) {
      ...
      if (item && item.title) {
        return res.json({
          ...
          category:    mapBarcodeCategory(item.category || ''),
          value:       item.lowest_recorded_price || item.msrp || 0
        });
      }
    }
  } catch {}

  // Fallback: Open Food Facts (no key, unlimited, great for groceries)
  try {
    const r = await withTimeout(fetch(`https://world.openfoodfacts.org/api/v0/product/${code}.json`), 5000);
    if (r.ok) {
      const data = await r.json();
